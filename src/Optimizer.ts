...
import {
  ...
  AmountsCallResult,
  GetInPathParams,
  GetOptimalInPathParams,
  GetOptimalOutPathParams,
  GetOutPathParams,
  MulticallCallDataInput,
  Pool,
  PoolReserves,
  Reserve,
...
class UniswapV2PathOptimizer {
  ...
  constructor({
    ...
    this._customFee = customFee ?? false;
  }
  ...
  public getTokenId(address: address): number {
    return this._tokens.findIndex(
      (t) => t.address === utils.getAddress(address)
    );
  }
  ...
  public async init(tokens: address[]) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      this._tokens.push(new Token(tokens[i], symbol, decimals));
  ...
  public async refresh() {
    ...
    if (this._customFee) {
      ...
      const poolFees = await this.multicall<bigint>(calls);
      poolFees.map((fee, i) => {
        this._pools[i].feeBps = Number(fee);
      });
    }
  }
  ...
  public async getOutPathsOnChain({
    ...
    const calls: MulticallCallDataInput[] = paths.map((path) => ({
    ...
    return this.toAmountsOutResult(
      paths,
      amountsOuts.map((outs) => outs.amounts),
      take
    );
  }

  public async getOptimalOutPathOnChain(
    props: GetOutPathParams
  ): Promise<AmountsOutResult> {
  ...
  public async getInPathsOnChain({
    ...
    return this.toAmountsInResult(
      paths,
      amountsIns.map((ins) => ins.amounts),
      take
    );
  }

  public async getOptimalInPathOnChain(
    props: GetOptimalInPathParams
  ): Promise<AmountsInResult> {
  ...
  private getReserves(tokenInId: number, tokenOutId: number): PoolReserves {
    const { feeBps, token0, reserve0, reserve1 } = this.getPoolByTokenId(
      tokenInId,
      tokenOutId
    );
    const [reserveIn, reserveOut] =
      token0 === tokenInId ? [reserve0, reserve1] : [reserve1, reserve0];
  ...
  public getAmountOut(
    ...
    tokenBId: number,
    amountIn: BigNumberish,
    priceImpact: boolean = true
  ) {
    const { feeBps, reserveIn, reserveOut } = this.getReserves(
      tokenAId,
      tokenBId
    );
  ...
  public getOutPathsOffChain({
    from,
    to,
    amountIn,
    maxLength,
    take,
  }: GetOutPathParams): AmountsOutResult[] {
    const paths = this.bfs(from, to, maxLength);
    const amountsOuts: BigNumber[][] = paths.map((path) =>
      this.getAmountsOut(amountIn, path)
    );
    return this.toAmountsOutResult(paths, amountsOuts, take);
  }

  public getOptimalOutPathOffChain(
    props: GetOptimalOutPathParams
  ): AmountsOutResult {
    return this.getOutPathsOffChain({ ...props, take: 1 })[0];
  }

  public getInPathsOffChain({
    from,
    to,
    amountOut,
    maxLength,
    take,
  }: GetInPathParams): AmountsInResult[] {
    const paths = this.bfs(from, to, maxLength);
    const amountsIns: BigNumber[][] = paths.map((path) =>
      this.getAmountsIn(amountOut, path)
    );
    return this.toAmountsInResult(paths, amountsIns, take);
  }

  public getOptimalInPathOffChain(
    props: GetOptimalInPathParams
  ): AmountsInResult {
